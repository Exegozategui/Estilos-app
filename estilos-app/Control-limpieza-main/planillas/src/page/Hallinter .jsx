...
const Hallinter = () => {
  ...
  const [banosPorPagina] = useState(1);
    ...
    const banosHallInter = banosData.baños.filter(bano => bano.sector.toLowerCase().includes('hall')&& bano.location.toLowerCase().includes('internacional') );
  

  const indexOfLastBano = currentPage * banosPorPagina;
  const indexOfFirstBano = indexOfLastBano - banosPorPagina;
  const banosActuales = banosHallInter.slice(indexOfFirstBano, indexOfLastBano);

  const totalPages = Math.ceil(banosHallInter.length / banosPorPagina);
