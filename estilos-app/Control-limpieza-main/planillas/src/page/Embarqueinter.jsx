...
const Embarqueinter = () => {
  ...
  const [banosPorPagina] = useState(1);
    ...
    const banosEmbarqueInter = banosData.baños.filter(bano => bano.sector.toLowerCase().includes('embarque')&& bano.location.toLowerCase().includes('internacional') );
  

  const indexOfLastBano = currentPage * banosPorPagina;
  const indexOfFirstBano = indexOfLastBano - banosPorPagina;
  const banosActuales = banosEmbarqueInter.slice(indexOfFirstBano, indexOfLastBano);

  const totalPages = Math.ceil(banosEmbarqueInter.length / banosPorPagina);
